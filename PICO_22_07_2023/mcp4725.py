...
class MCP4725:
    ...
    def write(self, value):
        if value < 0:
            value = 0
        value = value & 0xFFF  # Ensure value is within the 12-bit range
        
        # Construct the write buffer
        self._writeBuffer[0] = (value >> 8) & 0xFF  # Upper 8 bits
        self._writeBuffer[1] = ((value & 0xFF) << 4) | 0x0F  # Lower 4 bits + control bits
    ...
    def read(self):
        buf = bytearray(5)  # Create a buffer to store read data
        if self.i2c.readfrom_into(self.address, buf) == 5:
            # Parse the read data
            eeprom_write_busy = (buf[0] & 0x80) == 0
            power_down = self._powerDownKey((buf[0] >> 1) & 0x03)
